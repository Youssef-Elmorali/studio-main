...
export default function BloodBankDetailPage() {
  ...
  React.useEffect(() => {
    ...
    const fetchBankDetails = async () => {
      ...
      setError(null);
      try {
          const bankDocRef = doc(db, 'blood_banks', bankId);
        ...
        if (bankDocSnap.exists()) {
             ...
             const bankData: BloodBank = {
                 ...
                 locationCoords: data.locationCoords ? { lat: data.locationCoords.latitude, lng: data.locationCoords.longitude } : undefined, // Handle GeoPoint
                 contactPhone: data.contactPhone,
                 operatingHours: data.operatingHours,
                 website: data.website,
                 inventory: data.inventory as BloodInventoryMap || {}, // Ensure type assertion
                 lastInventoryUpdate: data.lastInventoryUpdate, // Keep as Timestamp
        ...
        setError("Failed to load blood bank details. Please try again.");
      } finally {
        setLoading(false);
      }
    };
    ...
    const apiKey = process.env.NEXT_PUBLIC_GOOGLE_MAPS_API_KEY || '';
     const initialGoogleMapsEmbedUrl = bank?.location // Use initial...
         ? apiKey
             ? `https://www.google.com/maps/embed/v1/place?key=${apiKey}&q=${encodeURIComponent(bank.location)}` // Use place query
       ...
       return (
           <div className="container mx-auto py-12 px-4 text-center">
               ...
               <Button asChild variant="outline">
                   <Link href="/request"><ArrowLeft className="mr-2 h-4 w-4" /> Back to Requests</Link>
               </Button>
           </div>
       );
   }
