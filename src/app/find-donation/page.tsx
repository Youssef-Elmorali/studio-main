...
export default function FindDonationPage() {
  ...
  // Format locations for map with validation
  const mapLocations = React.useMemo(() => {
    return filteredBanks.map(bank => {
      // Validate coordinates
      const coords = bank.locationCoords || { lat: 0, lng: 0 };
      const validCoords = {
        lat: typeof coords.lat === 'number' && !isNaN(coords.lat) ? coords.lat : 0,
        lng: typeof coords.lng === 'number' && !isNaN(coords.lng) ? coords.lng : 0
      };
  ...
  // Get center coordinates for selected city
  const mapCenter = React.useMemo(() => {
    if (selectedCity === 'all') return undefined;
    
    const city = EGYPTIAN_CITIES.find(city => city.name === selectedCity);
  ...
  const MapComponent = React.useMemo(() => (
    ...
    />
  ), [mapLocations, handleBankSelect, mapCenter]);
      ...
      <div className="flex flex-col md:flex-row gap-4 items-start md:items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-2">
            Locate blood banks and donation centers in your city.
          </p>
        </div>
            ...
            <Card>
              <CardContent className="p-6">
                <div className="flex flex-col items-center justify-center text-center space-y-2">
                  ...
                  <p className="text-muted-foreground">No donation centers found in this city.</p>
                </div>
              </CardContent>
            </Card>
          ) : (
